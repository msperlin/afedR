...
#' 
#' Now that we learned the basics of programming i
#' 
#' - Changing format of a dataframe (long/wide);
#' - Converting a `list` of `dataframes` into a si
#' - Identifying and treating extreme values (_out
#' - Price data disinflation;
#' - Changing time frequency.
#' 
#' 
#' ## The Format of a `dataframe`
#' 
#' The dataframe format is a discussion that arose
#' 
...
#' 
#' Its worth noting that, in finance, the wide for
#' 
#' 
...
#' 
#' For the first, lets use package `purrr` and a c
#' 
## ---- include=FALSE------------------------------------------------------
# clean up files
...
#' 
#' The files are available, as expected. Now, lets
#' 
## ------------------------------------------------------------------------
...
#' 
#' The first step is registering the actual names 
#' 
## ------------------------------------------------------------------------
...
#' 
#' From the output of `glimpse` we see that column
#' 
## ------------------------------------------------------------------------
...
#' 
#' To visualize the destructive effect of a _outli
#' 
...
#' 
## ------------------------------------------------------------------------
# find value of vector that sets 0.95 probability
...
#' 
#' Now, we can use this information to find the ex
#' 
## ------------------------------------------------------------------------
# find cases higher than 90% quantile
...
#' 
#' Finally, we need to treat the outliers. We can 
#' 
## ------------------------------------------------------------------------
# copy content
...
replace_outliers <- function(col_in, my_prob = 0.05) {
  ...
  #         my_prob Probability of quantiles (will remove quantiles at p and 1-p)
  #
...
# get data!
df_inflation <- get_Quandl_series(id_in = my_symbol,api_key = my_api_key, 
...
#' 
#' Now, lets create a random dataframe with random
#' 
## ------------------------------------------------------------------------
...
#' 
#' Sometimes we receive data in the wrong time fre
#' 
...
#' 
#' One of the ways to perform the calculation is t
#' 
## ------------------------------------------------------------------------
...
#' 
#' We create a new column with the years, group th
#' 
#' Users interested in knowing a more complete for
#' 
#' 
#' ## Exercises
#' 
...
#' 
#' 07. Use the same daily data from previous exerc
#' 
