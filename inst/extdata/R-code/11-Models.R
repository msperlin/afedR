...
#' 
#' The tools from financial econometrics allow the
#' 
...
#' 
#' As for the output of `lm`, it is important to u
#' 
## ------------------------------------------------------------------------
...
#' 
#' Its content is a simple atomic vector that incr
#' 
...
# calculate return
df_sp500$ret <- calc.ret(df_sp500$price)
...
my_df_asset$ret.sp500 <- df_sp500$ret[idx]
...
#' 
#' The output shows stock `r my_asset` has a beta 
#' 
#' 
...
#' 
#' As an example with real data, we'll use a credi
#' 
## ------------------------------------------------------------------------
library(tidyverse)
...
#' 
#' This is a comprehensive dataset, with several i
#' 
## ------------------------------------------------------------------------
library(tidyverse)
...
glm_credit <- glm(data=df_default, 
                   formula = default ~ D_male_gender +  age + educ + D_marriage,
                   family = binomial(link = "logit"))
...
#' 
#' One of the issues in working with Arima models 
#' 
## ------------------------------------------------------------------------
library(forecast)
...
#' 
#' In CRAN we can find two main packages related t
#' 
...
my_garchfit <- garchFit(data = sim_garch, 
                        formula = sim.ret ~ arma(1,0) + garch(1,1), 
                        trace = FALSE)
...
my_garchfit_SP500 <- garchFit(data = df_est , 
                              formula = ret.adjusted.prices ~ arma(1,0) + garch(1,1), 
                              trace = FALSE)
...
my_SP500_MS_model <- MS_Regress_Fit(dep, indep, S, k)	# fitting the model
...
p <- ggplot(df.to.plot,
  ...
  labs(x = '',
       ...
       subtitle = 'Probabilities from an estimated markov regime switching model',
       caption = 'Data from Yahoo Finance')
...
p <- ggplot(df_to_plot, aes(x = my_betas)) +
  ...
  labs(x = 'Betas',
       ...
       subtitle = paste0('Market models estimated with data from ',
                         ...
                         length(unique(df_stocks$ticker)), ' stocks included'),
       caption = 'Data from Yahoo Finance') + 
  theme_bw()
...
#' 
#' 02. Using package `car` and the previously simu
#' 
...
#' 
#' 06. For the same stock data from previous exerc
#' 
#' 07. Using the tidyverse functions `dplyr::group
#' 
#' 08. Using the same "pipeline" code as previous 
#' 
