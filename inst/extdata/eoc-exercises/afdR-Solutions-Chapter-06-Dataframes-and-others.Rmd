...
**03. Use function `dplyr::filter` and the _pipeline_ operator to filter `my_df`, so that it only keeps rows where the value of column `x` is greater than 30.**
...
**04. If not yet used, repeat exercises 1, 2 and 3 using the `tidyverse` functions and the _pipeline_ operator to chain all operations together.**
...
**05. Use package `BatchGetSymbols` to download Facebook (FB) stock data since `'2010-01-01'` until today. If an investor bought `r scales::dollar(1000)` worth of FB stock in the first available day after `'2010-01-01'`, what would be the value of his portfolio today?**
...
**06. Download the book data file available at the following link:**

[`r book.site.zip`](`r book.site.zip`)

**After unzipping its contents, import the data in file `grunfeld.csv`. Use functions `dplyr::glimpse` and `base::summary` to better understand the content of the imported dataset.**
...
my_folder <- '~/Desktop/Book files' # probably C:/Users/USERNAME/Desktop in Windows
unzip(local_file, exdir = my_folder)

my_file <- file.path(my_folder, 'R code and data (zip)/data/grunfeld.csv')
...
**07. Create a list object with three dataframes in its contents, `df1`,` df2` and `df3`. The content and size of dataframes is your choice. Use `sapply` function to find out the number of rows and columns in each dataframe.**
