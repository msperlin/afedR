...
# Chapter 07 - Basic Object Classes

**01. Let's assume that at certain date you bought 100 shares of a company, 15 dollars per share. After some time, you sold 30 shares for 18 dolars each and the remaining shares were sold for 22 dolars in a later day. Using a _script_ in R, structure this financial problem by creating numeric objects in R. What is the gross profit of this stock market transaction? **
...
**02. Consider the following numeric vectors `x` and `y`:**
...
**04. Define another object named `seq_2`, which contain a sequence of length 1000, with values between 0 and 100.**
...
**05. Calculate the sum between `seq.1` and `seq.2`. Did this operation work despite the different size of the vectors? Explain your answer.**
...
**06. If we created an object with the cumulative sum of a sequence from 1 to 100, what is the position of the first element to exceed the value of 50?**
...
**08. CHALLENGE - Create a $z_i$ vector according to the following formula, where $x_i = 1...50$ and $y_i = 50...1$. What is the value of the sum of $z_i$ elements? **
...
**12. At link [https://www.gutenberg.org/files/1342/1342-0.txt](https://www.gutenberg.org/files/1342/1342-0.txt) you can find a file containing the full text of Jane Austen's _Pride and Prejudice_ book. Use the link directly with function `readr::read_lines` to import the entire book as a vector of characters called `my_book`. How many lines does the resulting object has? **

```{r}
my_link <- 'https://www.gutenberg.org/files/1342/1342-0.txt'
...
**13. Aggregate the vector of characters in `my_book` to a single text object named `full_text` using function `paste0(my_book, collapse = '\n')`. With functions from package `stringr`, check how many times the word `'Bennet'` -- one of the protagonists -- repeats itself throughout the text. **
...
**14. CHALLENGE - Use function `stringr::str_split` in object `full_text` to break the whole text as a function of whitespace (`' '`). Use this information with function `base::table` and create a frequency table, showing the number of ocurrences of each word. What are the five most used words in the text? **
...
**15. Assuming you'll live for 100 years, what is the proportion of your birthdays days that will land on a weekend day (sunday/saturday)? Tip: Use operator `%in%` to check a multiple equality condition.**
