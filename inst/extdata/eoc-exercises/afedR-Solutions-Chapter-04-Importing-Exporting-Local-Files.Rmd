...
# Chapter 04 - Importing and Exporting Data from Local Files
...
**Export the resulting dataframe to each of the following formats:**
...
**03. Within the previous code, change the value of `my.N` to `1000000` and re-execute the code. Does this change modifies the answers of the two previous questions? How?**
...
**04. In the [book site](`r book.site`) you can find a [downloadable zip file](`r book.site.zip`) with all data files used to compile this book. Using function `readr::read_csv`, read the contents of file `'R code and data (zip)/data/SP500-Stocks_long.csv'`. How many columns and how many rows you find in this table?**

```{r, cache = TRUE}
my_url <- 'https://www.msperlin.com/blog/files/pafdr%20files/Code_Data_pafdR.zip'
temp_zip <- tempfile(pattern = 'book_zip_file', fileext = '.zip')

# download file
download.file(my_url, temp_zip)

# unzip to temp folder
folder_to_unzip <- file.path(tempdir(), 'book folder')
dir.create(folder_to_unzip)
unzip(zipfile = temp_zip, exdir = folder_to_unzip)

# file to read
my_file <- file.path(folder_to_unzip, 
                     'R code and data (zip)',
                     'data', 
                     'SP500-Stocks_long.csv')

df_SP500 <- readr::read_csv(my_file, col_types = cols())

my_msg <- paste0('There are ', nrow(df_SP500), ' rows and ', 
                 ncol(df_SP500), ' columns in file ', basename(my_file))
cat(my_msg)
```
...
**you can find data about all baby names in France from 1900 to 2017. But, the data comes with a caveat: all columns are separated by a tab symbol. After reading the manual for `readr::read_delim`, import the information from this file in your computer. How many rows does the file contains? **
