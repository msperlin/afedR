...
Load the data from files \Sexpr{basename(br_stocks_file)} and \Sexpr{basename(f_ibov_name)}. Now, using the Ibovespa returns (ticker = BVSP), add a new column in the stock data dataframe with the returns of the index. For each asset, calculate the excessive return of each stock for each date, that is, the difference of the stock return and the index return.
