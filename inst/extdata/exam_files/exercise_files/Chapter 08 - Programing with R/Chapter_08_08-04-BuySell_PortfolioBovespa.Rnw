...
temp <- filter(df_br_stocks, ticker %in% traded.assets)

df.oper <- temp %>% group_by(ticker) %>%
  summarise(buy.date = sample(ref.date[1:floor(n()/2)],1),
...
for (i in seq(1,n.oper)){
  ...
  temp <- filter(df_br_stocks, ticker==df.oper$ticker[i])
...
Consider the raw table from file \Sexpr{basename(br_stocks_file)}. Based on it, assume that a investor realized the following trade operations:
